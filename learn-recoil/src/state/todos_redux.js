...
// seletor => 상태값들 중 특정 값을 고르거나 연산해서 리턴
...
export const getPercentage = createSelector(
  ...
  (totalCount, undoneCount) => {
    ...
    return Math.floor((undoneCount / totalCount) * 100);
  }
);
...
export const removeTodoAction = createAction("toods/remove");
...
const todosSlice = createSlice({
  ...
  reducers: {
    ...
    toggle(state, action) {
      const { id } = action.payload;
      // 객체는 참조 복사이기 때문에 mutable하게 변경해서 return 값 x.
