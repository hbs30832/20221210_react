...
if (true) {
  var num = 100;
  var num02 = 200; // 블록 레벨 스코프 지원하지 않기 때문에 전역 변수 num02가 생성된다.
  console.log(num);
}
...
// 템플릿 리터럴 : ``(백틱) ${표현식}을 통해 문자열 안에 표현식을 포함시킬 수 있다.
...
// &&(and) : 앞의 값이 truthy 한 값이라면 뒤에 값으로 평가되고, 앞의 값이 falsy한 값이라면 앞의 값으로 평가된다.
let color = "dsajkldsjlk" && "red";
...
// ??(nullish) : ?? 앞의 null이나 undefined이면 ?? 뒤의 값을 평가되고 그 외의 값이면 앞의 값으로 평가된다.
...
//  => 무조건 매개 변수의 마지막에 사용해야한다.
function getTotal(v, d, f, ...numList) {
